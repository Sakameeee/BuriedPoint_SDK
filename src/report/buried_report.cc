...
public:
  BuriedReportImpl(std::shared_ptr<spdlog::logger> logger,
      ...
      : logger_(std::move(logger)), common_service_(std::move(common_service)),
  ...
  // 开启一个定时器，五秒之后执行 ReportCache_ 方法
  void Start();
...
private:
  ...
  // 创建 json 对象存放数据，序列化加密后存入数据库
  BuriedDb::Data MakeDbData_(const BuriedData &data);

  // 根据缓存获取解密后的 json 字符串数据
...
private:
  ...
  // 装载要上报的数据，会不停的从数据库中按优先级取数据，装载到这个cache中
...
std::string
BuriedReportImpl::GenReportData_(const std::vector<BuriedDb::Data> &datas) {
